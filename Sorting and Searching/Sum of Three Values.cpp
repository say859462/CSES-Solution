...
// };
int n, m, x, y, k;
// Greedy pick shorter  duration time first .
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int target = k - num[i].first;
        int left = 0, right = n - 1;
        while (left < right)
        {
            if (target == num[left].first + num[right].first && num[left].second != num[i].second && num[right].second != num[i].second)
            {
                cout << num[i].second << " " << num[left].second << " " << num[right].second << "\n";
                return;
            }

            if (target < num[left].first + num[right].first)
            {
                right--;
            }
            else
