...
{
    cin >> n;
    vector<node> customer(n);
    vi ans(n);
    FOR(0, n - 1)
    {
        cin >> x >> y;
        customer[i] = node(i, x, y);
    }
    sort(all(customer));
    pq<pii> q; // store the departure time and room number
    int room = 0;
    int last_room = 0;
    FOR(0, n - 1)
    {
        ...
        {
            last_room++;
            q.push(mp(-customer[i].r, last_room)); // use negative number to maintain min-heap
            ans[customer[i].id] = last_room;
        }
        else
        {
            pii top = q.top();
            if (-top.first < customer[i].l)
            { // replace the room to the new customer
                q.pop();
                top.first = -customer[i].r;
                ans[customer[i].id] = top.second;
                q.push(top);
            }
            else
            {
                last_room++;
                ans[customer[i].id] = last_room;
                q.push(mp(-customer[i].r, last_room));
            }
        }
        room = max(room, sz(q));
    }
